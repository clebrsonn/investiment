...
public class StockSplitService extends AbstractService<StockSplit, String> {
    ...
    public StockSplit validateSave(StockSplit stockSplit) {
        var investments= investmentRepository.findAllByStockCodeAndDateLessThanEqual(stockSplit.getName(), stockSplit.getDate());
        var investmentsToSave = investments.parallelStream().peek(investment -> {
           ...
           investment.setValue(investment.getValue().divide(stockSplit.getTo().divide(stockSplit.getFrom(),4, RoundingMode.HALF_UP),4, RoundingMode.HALF_UP));
